	...
	Component({
	  ...
	  methods: {
	    ...
	    bindMoreTap: async function(e) {
	      ...
	      wx.showActionSheet({ itemList: [orderDesc, vouchDesc, '生成专属二维码'],
	        success: async function(res) {
	          switch(res.tapIndex) {
	            case 0: var s = (order == 0) ? 9999 : 0; e.currentTarget.dataset['sort'] = s; await self._setSort(e); break;
	            case 1: vouch = (vouch == 0) ? 1 : 0; e.currentTarget.dataset['vouch'] = vouch; await self._setVouch(e); break;
	            case 2: var qrVal = dom.dataset(e, 'qr'); var title = encodeURIComponent(dom.dataset(e, 'title')); if (qrVal) { wx.navigateTo({ url: '/pages/admin/setup/qrcode/admin_setup_qrcode?title=' + title + '&qr=' + encodeURIComponent(qrVal) }); } else { wx.navigateTo({ url: '/pages/admin/setup/qrcode/admin_setup_qrcode?title=' + title }); } break;
	          }
	        }
	      });
	    },
